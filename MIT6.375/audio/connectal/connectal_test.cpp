...
// (1) the thread that sends request to HW and (2) another thread that processes
// indications from HW
pthread_mutex_t outpcmLock;
FILE *outpcm = NULL;

// The seperate thread in charge of indications invokes these call-back
// functions
class MyDutIndication : public MyDutIndicationWrapper {
public:
  // You have to define all the functions (indication methods) defined in
  // MyDutIndication
...
void run_test_bench() {
  ...
  // Just in case put seven more 0s for padding -- soft reset required for
  // subsequent usage
  //  8-POINT FFT
  for (int j = 0; j < 7; j++) {
...
int main(int argc, const char **argv) {
  float factor = 2.0;
  std::cout << "Enter a factor: " << endl;
  std::cin >> factor;
  uint16_t m_i = (uint16_t)floor(factor);
  uint16_t m_f = (uint16_t)((factor - m_i) * (1 << 16));
  uint32_t factor_int = (uint32_t)(m_i << 16) | m_f;

  // Service Indication messages from HW - Register the call-back functions to a
  // indication thread
